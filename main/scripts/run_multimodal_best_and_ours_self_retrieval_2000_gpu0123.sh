...
OURS_RECORD=results/behavior_analysis/qwen_residual_full_seed1_s20_b24/best_record.json
OURS_PROMPT_PATH=results/tmp_prompts/qwen_residual_full_seed1_s20_b24_best_prompt.txt

python - <<PY
import json
from pathlib import Path

record_path = Path("${OURS_RECORD}")
out_path = Path("${OURS_PROMPT_PATH}")
...
print(f"Saved ours prompt: {out_path}")
print(prompt[:500])
PY
...
echo "=== Inference: Ours behavior residual best ==="
run_infer "$OURS_PROMPT_PATH" \
  qwen_behavior_residual_best \
  behavior_residual_best \
  "Behavior Residual Best"

echo "=== Retrieval eval ==="
run_eval qwen_textgrad_multimodal_best siglip cuda:0 &
run_eval qwen_textgrad_multimodal_best clip   cuda:1 &
run_eval qwen_behavior_residual_best   siglip cuda:2 &
run_eval qwen_behavior_residual_best   clip   cuda:3 &
wait
...
save_path = base / "combined_summary_multimodal_best_and_ours.csv"
